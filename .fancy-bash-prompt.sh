...
bash_prompt_command() {
	...
	# Which length to use
	pwdmaxlen=$(( ( pwdmaxlen < ${#di} ) ? ${#dir} : pwdmaxlen ))
...
bash_prompt() {
	...
	local PROMT_TIME=$"$TEXT_FORMAT_1 \T "
	local PROMT_BAT=$"$TEXT_FORMAT_2  "
	...
	*)
	...
	############################################################################
	PS1="$TITLEBAR\n${PROMT_TIME}${SEPARATOR_1}`battery_prompt`${SEPARATOR_2}${PROMT_PWD}${SEPARATOR_3}${PROMT_INPUT}"
